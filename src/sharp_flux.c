/* Determines the magnetic flux in the umbra and penumbra of both polarities within
 * a sunspot group.
 * 
 * 
 * Changelog:		
 * 			
 * 			2014.06.20		--		First version of this code.
 * 
...
char *module_name="sharp_flux";
char *module_desc="determines the magnetic flux of sunspots within an active region by using SHARPs data. Outputs data to ARnumber.flux.dat.";
char *version_id="1.0";
...
int DoIt(){
    ...
    fitsfile *c_ptr,*i_ptr,*f_ptr;		/* Pointers for FITS files */
    fitsfile *info_ptr;	
    int c_status, i_status, f_status;	/* Status checker for opening files */
    int info_status;
    ...
    char f_filename[DRMS_MAXPATHLEN+1],info_filename[DRMS_MAXPATHLEN+1];
    ...
    double *p_pen_flux,*n_pen_flux;	/* Flux of field within penumbra */
    double *p_umb_flux,*n_umb_flux;	/* FLux of field within umbra */
	...
	p_pen_flux=malloc(num_records*sizeof(double));
	n_pen_flux=malloc(num_records*sizeof(double));
	
	p_umb_flux=malloc(num_records*sizeof(double));
	n_umb_flux=malloc(num_records*sizeof(double));	
	...
	for (k=0;k<num_records;k++){ 
		...
		if (time_is_invalid(t_obs)){
			...
			if (k == 0){
				...
				sprintf(outfile_name,"AR%d.flux-output.dat",noaa_ar);
			...
			if (quality[k] == 0){
				...
				* possibly due to icc and multithreading. */
				...
				info_dat=malloc(naxis2*sizeof(double *));
				radius=malloc(naxis2*sizeof(double *));
				area_map=malloc(naxis2*sizeof(double *));
				...
				info_pixel_strip=calloc(naxis1,sizeof(double));
	
				for (j=0;j<naxis2;j++){

					field_dat[j]=calloc(naxis1,sizeof(double));
					area_map[j]=calloc(naxis1,sizeof(double));
				...
				for (j=0;j<naxis2;j++){
					...
					fits_read_pix(i_ptr,TDOUBLE,fpixel,nelements,&nulval,i_pixel_strip,&any_nulls,&c_status);
				...
				if (num_good > 0){
					...
					/* This is a lot like the area code. First, we loop through and determine the area, incrementing 
					 * a 2-D array called area_map to keep track of the area on a pixel by pixel basis. As we do this, 
					 * we also count the number of times the pixel is mapped so that we can average the field over that
					 * area in order to calculate true magnetic flux. */ 
							...
							* cut a touch from the boundaries so we don't get wierd boundary issues. */
							...
							if ((patch_x < naxis1)&&(patch_x >= 0)&&(patch_y < naxis2)&&(patch_y >= 0)){
								...
								if (radius[patch_y][patch_x] < rsun_pix){
									...
									/* Add up the area for each pixel that is projected and lies within a sunspot 
									 * region. */
									 
								
									if ((con_dat[patch_y][patch_x] <= pen)&&(con_dat[patch_y][patch_x] > 0)){
										
										area_map[patch_y][patch_x]+=pixel_area(lat,lon,dpix);
									}
								}
							}
						}
					...
					/* Next, loop over naxis1 and naxis2 to calculate the flux. */
					
					for (i=0;i<naxis1;i++){
						
						for (j=0;j<naxis2;j++){
							
							if ((area_map[j][i] > 0)&&(field_dat[j][i] > cutoff)){
								
								/* Umbra */
								
								if ((con_dat[j][i] <= umb)&&(con_dat[j][i] > 0)){
									...
									if (inc_dat[j][i] < 90){
										...
										p_umb_flux[k]+=field_dat[j][i]*area_map[j][i]*fabs(cos(deg2rad*inc_dat[j][i]));
									...
									if (inc_dat[j][i] > 90){
										...
										n_umb_flux[k]-=field_dat[j][i]*area_map[j][i]*fabs(cos(deg2rad*inc_dat[j][i]));
								...
								if ((con_dat[j][i] <= pen)&&(con_dat[j][i] > umb)){
									...
									if (inc_dat[j][i] < 90){
										...
										p_pen_flux[k]+=field_dat[j][i]*area_map[j][i]*fabs(cos(deg2rad*inc_dat[j][i]));
									...
									if (inc_dat[j][i] > 90){
										...
										n_pen_flux[k]-=field_dat[j][i]*area_map[j][i]*fabs(cos(deg2rad*inc_dat[j][i]));
	...
	fprintf(outptr,"Active Region: %d NOTE: Flux is in Mx \n",noaa_ar);
	fprintf(outptr,"Full UT Time            time(s)  p umb flux  n umb flux  p pen flux  n pen flux\n");
	fprintf(outptr,"----------------------  -------  ----------  ----------  ----------  ----------\n");
	...
	for (i=0;i<num_records;i++){
		...
		if (quality[i] == 0){
			...
			fprintf(outptr,"%s  %7ld  %7.2lf  %7.2lf  %7.2lf  %7.2lf\n",t_obs_s[i],t_secs[i]-t_secs[0],p_umb_flux[i]*10000.0,n_umb_flux[i]*10000.0,p_pen_flux[i]*10000.0,n_pen_flux[i]*10000.0);
